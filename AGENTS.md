...
- **Sanctioned exception — GSAP**: [GSAP](https://gsap.com) drives the layered "staggered menu" drawer animation in `modules/staggeredMenu.js`. That module self-loads GSAP as a CDN ES module (`https://cdn.jsdelivr.net/npm/gsap@3.12.5/+esm`) via its own `ensureGSAP()` loader, preferring an existing `window.gsap` first. `index.html` also eager-loads `gsap.min.js` from the CDN so the menu can animate on first open without a lazy-load delay. The module degrades to a non-animated fallback if GSAP fails to load — keep that fallback intact. No npm/build step is introduced.
